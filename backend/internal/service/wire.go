...
func ProvideNVIDIAInferencePrewarmer(upstream HTTPUpstream, accountRepo AccountRepository, settingService *SettingService, metrics *NVIDIASharedConnectionPoolMetrics, blockChecker nvidiaPrewarmBlockChecker) *NVIDIAInferencePrewarmer {
...
func ProvideRateLimitService(
	...
	nvidiaThrottleCache NvidiaAdaptiveThrottleCache,
) *RateLimitService {
...
func ProvideOpsService(
	...
	nvidiaRateLimit *RateLimitService,
	nvidiaPoolMetrics *NVIDIASharedConnectionPoolMetrics,
	nvidiaPrewarmer *NVIDIAConnectionPrewarmer,
) *OpsService {
	svc := NewOpsService(
		opsRepo,
		settingRepo,
	...
	svc.SetNVIDIAMetricsSources(nvidiaRateLimit, nvidiaPoolMetrics, nvidiaPrewarmer)
...
// ProvideGatewayService wraps NewGatewayService to inject the NVIDIA shared
// connection pool metrics required by the W3 scheduling-locality sort tiebreak.
// Wire-only post-construction wiring step; must not be called after start.
func ProvideGatewayService(
	...
	userPlatformQuotaRepo UserPlatformQuotaRepository,
	nvidiaSharedPoolMetrics *NVIDIASharedConnectionPoolMetrics,
) *GatewayService {
	...
	svc.SetNVIDIASharedPoolMetricsForSorting(nvidiaSharedPoolMetrics)
	return svc
}

// ProvideOpenAIGatewayService wraps NewOpenAIGatewayService to inject the
// NVIDIA shared-connection-pool metrics so the fastpath (handleOpenAIAccountUpstreamError)
// can record upstream failures (N3) via RecordAccountFail. N3 mirrors the wiring
// already done for GatewayService by W5.
...
var ProviderSet = wire.NewSet(
	...
	wire.Bind(new(nvidiaPrewarmBlockChecker), new(*OpenAIGatewayService)),
	NewOAuthService,
	...
	NewNVIDIASharedConnectionPoolMetrics,
)
