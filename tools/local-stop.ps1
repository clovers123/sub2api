...
function Stop-Port {
  ...
  try {
    ...
    $NetstatMatches = (netstat -ano -p tcp 2>$null) | Select-String -Pattern $Pattern
    foreach ($M in $NetstatMatches) {
      $ProcId = [int]$M.Matches[0].Groups[1].Value
      Write-Host "Stopping $Label listener on port $Port ($ProcId)..."
