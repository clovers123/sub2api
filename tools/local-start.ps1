...
function Invoke-Shutdown {
  if ($Global:ShutdownRequested) { return }
  $Global:ShutdownRequested = $true
  Write-Host "`nShutdown requested, stopping services..."
}

Register-EngineEvent -SourceIdentifier PowerShell.Exiting -Action { Invoke-Shutdown } | Out-Null
...
if (-not $SkipBuild) {
  ...
  Write-Host "Skipping backend build (--SkipBuild)"
}
